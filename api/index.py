...
def _pa_wsgi_path() -> str:
    ...
    # account has capitals (e.g. "AvetTumo"), so match on the lowercased name
    # or the post-deploy reload would touch a file PA never reads and silently
    # no-op.
    for user in users:
...
def deploy():
    ...
    try:
        ...
        try:
            ...
            webhook_line = f"Webhook/command registration failed: {e}"
