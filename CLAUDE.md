...
- **`/video <prompt>`** — **text-to-video only** (image-to-video was removed on request), via a free Hugging Face Space (`HF_VIDEO_SPACE`, default `Lightricks/ltx-video-distilled`). `cmd_video` → `_do_video(message, prompt)` → `_generate_video()` → `_generate_video_hf()`.
  - **Backend (`_generate_video()` → `_generate_video_hf()`):** the *only* free, PA-allowlist-compatible video backend is the HF Space (reached via `gradio_client` over `hf.space`). Because video Spaces have very different gradio signatures, `_build_video_call(prompt)` holds a per-Space call profile keyed on the Space id and returns `(args, api_name)`, passed **positionally** (gradio param names vary by version). Profiles: LTX family → `/text_to_video`, 13-arg; CogVideoX → `/generate`, 7-arg (see the `HF_VIDEO_SPACE` env-table row). Returns an mp4 sent via `send_video` (falls back to `send_document`).
  - **No keyless fallback:** Together/Cloudflare have no free text-to-video, and pollinations is text-to-image only, so blanking `HF_VIDEO_SPACE` disables `/video` with a clear "not configured" message. Video is heavier than an image edit — on the shared free GPU a clip can take a minute or more; `_do_video` wraps the call in `keep_typing(chat_id, action="upload_video")` and delivery is out-of-band, so a slow clip still arrives. Backend exceptions are logged (`print("Error in _do_video: …")`). Free video Spaces are more volatile than the FLUX Kontext one — if `/video` breaks, re-verify the target Space's api_name + arg order (`client.view_api()`) and adjust the profile in `_build_video_call`.
