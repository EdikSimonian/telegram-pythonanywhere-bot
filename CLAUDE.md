...
| `HF_VIDEO_SPACE` | No | `Lightricks/ltx-video-distilled` | Hugging Face Space (LTX-Video) used by `/video` — the **only** free, PA-allowlist-compatible video backend (reached via `gradio_client` over `hf.space`). The default Space exposes two endpoints, `/text_to_video` and `/image_to_video`, sharing one ordered signature (`_generate_video_hf` calls them positionally). Blank it to disable `/video` — there is **no** keyless video fallback (Together/Cloudflare don't offer free text-to-video). To swap Spaces, confirm the same two api_names + arg order or adapt `_generate_video_hf` |
