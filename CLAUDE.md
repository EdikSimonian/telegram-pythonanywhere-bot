...
| `HF_TOKENS` | No | — | Comma-separated extra HF tokens for `/video` quota rotation. `_hf_video_tokens()` tries `HF_TOKEN` first, then these in order; on a ZeroGPU **quota** error `_generate_video_hf()` rolls over to the next token, so N tokens ≈ N× the free daily video budget (a non-quota error still fails fast). **Caveat:** creating extra HF accounts solely to exceed the free quota may violate Hugging Face's terms — set at your own discretion |
...
| `HF_VIDEO_SPACE` | No | `Lightricks/ltx-video-distilled` | Hugging Face Space used by `/video` — the **only** free, PA-allowlist-compatible video backend (reached via `gradio_client` over `hf.space`). Blank it to disable `/video` — there is **no** keyless video fallback (Together/Cloudflare don't offer free text-to-video). Because video Spaces have very different gradio signatures, `_build_video_call()` holds a per-Space **call profile** keyed on the Space id: **LTX** family (`Lightricks/ltx-video-distilled`, default) → fast distilled model, `/text_to_video`+`/image_to_video`, 13-arg signature, honors `HF_VIDEO_DURATION/GUIDANCE/IMPROVE`; **CogVideoX** (`zai-org/CogVideoX-5B-Space`) → full 5B model, far better *prompt adherence* but slower (~3-5 min) and heavier on ZeroGPU quota, `/generate`, 7-arg signature, fixed ~6s length + internal guidance (so the `HF_VIDEO_*` knobs don't apply). To add another Space, add a branch to `_build_video_call` after confirming its api_name + arg order via `client.view_api()` |
| `HF_VIDEO_TIMEOUT` / `HF_VIDEO_DURATION` / `HF_VIDEO_GUIDANCE` | No | `300` / `2` / `3.0` | `/video` knobs: gradio_client timeout (s, generous for the ZeroGPU queue), clip length (s — keep small; longer = slower + more timeouts), guidance scale |
| `HF_VIDEO_HEIGHT` / `HF_VIDEO_WIDTH` | No | `512` / `704` | `/video` frame size. Match the Space's defaults; the model prefers multiples of 32 |
| `HF_VIDEO_IMPROVE` | No | _(off)_ | `/video` two-pass "improve texture" render. On ~doubles the GPU seconds per clip (and the free daily ZeroGPU quota it burns) for a modest quality bump. Off (default) uses the fast single-pass path — quicker clips, roughly 2× as many per day on the free quota. Set to `1`/`true` for higher quality |
...
- **`/video <prompt>`** — text-to-video **and** image-to-video, via a free Hugging Face Space running LTX-Video (`HF_VIDEO_SPACE`, default `Lightricks/ltx-video-distilled`). Two entry modes: (1) a bare `/video <prompt>` → text-to-video (`_do_video` with `file_id=None`); (2) image-to-video — send a photo captioned `/video <optional motion>` (handled by `cmd_video_caption`, gated on `_command_in_caption(m, "video")`, the same caption trick `/edit` uses) **or** reply to a photo/image with `/video <optional motion>`. When the image path has no words, `_do_video` supplies a gentle default motion prompt.
  - **Backend (`_generate_video()` → `_generate_video_hf()`):** the *only* free, PA-allowlist-compatible video backend is the HF Space (reached via `gradio_client` over `hf.space`; the source image, if any, is uploaded straight to the Space — no public URL / token leak, same as `/edit`). The default Space exposes `/text_to_video` and `/image_to_video` sharing one 13-arg ordered signature; `_generate_video_hf` passes args **positionally** (gradio param *names* vary by version) and picks the api_name by whether an image was given. Returns an mp4 sent via `send_video` (falls back to `send_document`).
  - **No keyless fallback:** Together/Cloudflare have no free text-to-video, and pollinations is text-to-image only, so blanking `HF_VIDEO_SPACE` disables `/video` with a clear "not configured" message. Video is heavier than an image edit — on the shared free GPU a clip can take a minute or more; `_do_video` wraps the call in `keep_typing(chat_id, action="upload_video")` and delivery is out-of-band, so a slow clip still arrives. Backend exceptions are logged (`print("Error in _do_video: …")`). Free video Spaces are more volatile than the FLUX Kontext one — if `/video` breaks, re-verify the target Space's api_names + arg order (`client.view_api()`) and adjust `_generate_video_hf`.
  - **Free ZeroGPU quota is the real limit, not a bug.** The Space reserves ~60s of GPU per call (`get_duration()` in its `app.py`) and charges the HF account's *daily* free ZeroGPU budget (a few minutes/day even when authenticated with `HF_TOKEN`), so expect only a handful of clips/day before it's exhausted until the ~24h reset. `_do_video` detects this case via `_is_zerogpu_quota_error()` and shows a friendly "out of free video quota, try again later" message instead of the raw HF PRO upsell URL. Levers to stretch it: set `HF_TOKENS` to rotate through several tokens (`_generate_video_hf` rolls over to the next on a quota error — see the env table caveat), keep `HF_VIDEO_DURATION` small, leave `HF_VIDEO_IMPROVE` off (single-pass ≈ half the GPU seconds), or point `HF_VIDEO_SPACE` at a Space on a dedicated (non-ZeroGPU) GPU. More reliable throughput needs HF PRO or a paid video API.
